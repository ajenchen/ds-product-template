...
This updates the npm packages (DS + 治理本體 in `ds-canonical/fork`) and idempotently refreshes the committed governance skeleton (launchers + `settings.json` hooks block, removing obsolete plugin-era hooks). Restart Claude Code after it succeeds. **No plugin involved (C-prime committed-config).**
...
`sync-all` 做兩件:(1) `npm install @beta` 更治理本體(`ds-canonical/fork`:fork hooks + preamble + manifest);(2) 從 npm idempotent 刷新接線骨架(committed 啟動器 + `settings.json` hooks 區塊,移除 obsolete plugin-era hook,帶 `.github/no-governance-sync` opt-out)。重啟 session 生效。
