...
**Human-only boundaries**:僅 login/MFA/OAuth/owner/billing、缺 credential reference(只問 vault/Secret Manager 的 reference,禁 secret)、plan 外付費、法律／帳號／組織權限／商業承諾,以及上述產品決策。Agent 先完成唯一方案與 preflight,只問**一個 exact action**,readback 後續跑;technical failure 一律 fail-closed,不是 human decision。
